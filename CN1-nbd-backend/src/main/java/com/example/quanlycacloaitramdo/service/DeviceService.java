        ...
        try {
            ...
            if (deviceValidator.isValid(device)) {
                if (deviceRepository.findByObjectTYPE(device.getObject_TYPE().toUpperCase()) == 0) {
                    ...
                    logsActionRepository.save(new LogsAction(10077, "Quản lý loại trạm đo", "ADD", "administrator", today, "OBJECT_TYPE"));
    ...
    public ResponseEntity<Device> updateDevice(Device device) {
        try {
            ...
            if (deviceValidator.isValid(device)) {
                ...
                logsActionRepository.save(new LogsAction(10779, "Quản lý loại trạm đo", "UPDATE", "administrator", today, "OBJECT_TYPE"));
                return new ResponseEntity<Device>(_device, HttpStatus.CREATED);
            } else return null;
        } catch (Exception e) {
            return new ResponseEntity<>(null, HttpStatus.INTERNAL_SERVER_ERROR);
